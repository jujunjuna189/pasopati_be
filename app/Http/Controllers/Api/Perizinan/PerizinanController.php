    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($codeLength > 0) {
                ...
                $checkData = PerizinanModel::where($wherePer)->whereDate('created_at', Carbon::now())->get();
                if ($checkData->count() > 0) {
                    PerizinanModel::where($wherePer)->whereDate('created_at', Carbon::now())->update($data);
                    ...
                    $response = PerizinanModel::create($data);
                }
            } else {
                $response = false;
            }
